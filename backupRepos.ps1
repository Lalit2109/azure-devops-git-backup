...
try {
    ...
    foreach ($repo in $repos) {
        ...
        try {
            ...
            # Get authenticated URL (handles URLs with or without existing credentials)
            $url = Get-AuthenticatedUrl -Url $remoteUrl -AccessToken $AccessToken
    ...
    if ($RetentionDays -gt 0) {
        ...
        Write-Log "Cutoff date: $($cutoffDate.ToString('yyyy-MM-dd')) - backups older than this will be deleted" "INFO"
        
        try {
            # Get all backup zip files (exclude manifest files)
            ...
            foreach ($blob in $allBlobs) {
                try {
                    # Extract date from filename: ProjectName/RepoName/YYYY-MM-DD.zip
                    $fileName = $blob.Name.Split('/')[-1]
                    $dateString = $fileName.Replace('.zip', '')
                    
                    # Parse the date from filename
                    $blobDate = [DateTime]::ParseExact($dateString, 'yyyy-MM-dd', $null)
                    
                    # Only delete if backup date is older than cutoff
                    if ($blobDate -lt $cutoffDate) {
                        Write-Log "Deleting old backup: $($blob.Name) (backup date: $dateString)" "INFO"
                    ...
                    Write-Log "Skipping file with invalid date format: $($blob.Name)" "WARNING"
                }
            }
            
            Write-Log "Retention policy applied: $deletedCount backups deleted, $keptCount backups kept" "SUCCESS"
        } catch {
